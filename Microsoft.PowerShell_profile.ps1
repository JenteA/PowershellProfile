...
Set-PSReadLineKeyHandler -Key Shift+Tab -Function ForwardChar 
...
#>
    ...
    param(
        ...
        [System.String]
        $Program = (Join-Path -Path $PsHome -ChildPath 'pwsh.exe'),
    ...
    elseif ($command -and ($program -match 'pwsh.exe$'))
    {
    ...
    if ($NoExit -and $program -match 'pwsh.exe$')
    {
